...
downloadpath='/data/ccaa' #Aria2下载目录
name='GoogleDrive' #配置Rclone时填写的name
folder='/杂物/未归档' #网盘里的文件夹，留空为整个网盘。
...
zipfile(){
	zip -r -j "${mypath}/压缩包.zip" "${mypath}" >>/root/log.txt
}
...
if [ "$path" = "$filepath" ] && [ $2 -eq 1 ] #普通单文件下载，移动文件到设定的网盘文件夹。
    ...
    then
        ...
        remotepath="${name}:${folder}/${downloadpath#${mypath}}/"
        echo "${mypath}" >>/root/ccaa-master/log.txt
        
        mv "${mypath}" /data/GoogleDrive/杂物/未归档/ >>/root/ccaa-master/log.txt
